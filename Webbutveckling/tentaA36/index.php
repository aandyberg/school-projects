...
<div id="container">
    ...
    <main>
        ...
        <?php 
                ...
                $get_books = "SELECT * FROM Books WHERE buyOrSell = 'sell'";
                ...
                if($result) {
                    while($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo '<p id="'.$row["bookImage"].'">Title: <a href="javascript:void(0)" id="'.$row["bookImage"].'" onclick="showText(this.id)">'.$row["title"].'</a></p>';
                        echo '<p>Pris: '.$row["price"].'</p>';
                        echo '<p>ISBN: '.$row["isbn"].'</p>';
                        echo '<p>Säljare: '.$row["sellerName"].'</p>';
                        echo '<p class="article-text" id="article-text'.$row["bookImage"].'">'.$row["comments"].'</p>';
                        echo '</div></div>';
                    }
                }
            ?>
